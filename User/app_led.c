...
static void App_Led_Uv_On_Callback(void *arg );
...
static uint8_t ledUvTimerId = TIMER_NULL;
...
static void App_Led_Handler(void *arg )
{
    ...
    if(App_Batt_Get_Usb_State() == USB_PLUG_IN && App_Key_Get_Hall_State() == CASE_CLOSE )
    {
        if(ledUvTimerId == TIMER_NULL)
        {
            App_Led_Uv_On();

            ledUvTimerId  = Drv_Timer_Regist_Oneshot(App_Led_Uv_On_Callback, 60000, NULL);
        }
    }
    else
    {
        if(ledUvTimerId != TIMER_NULL)
        {
            ...
            Drv_Timer_Delete(ledUvTimerId);

            ledUvTimerId = TIMER_NULL;
        }
    }
}
...
void App_Led_Uv_On(void )
{
    Drv_Led_Uv_On();
}

void App_Led_Uv_Off(void )
{
    Drv_Led_Uv_Off();
}
